/**
 * Definition for a binary tree node.
 * public class TreeNode {
 *     int val;
 *     TreeNode left;
 *     TreeNode right;
 *     TreeNode() {}
 *     TreeNode(int val) { this.val = val; }
 *     TreeNode(int val, TreeNode left, TreeNode right) {
 *         this.val = val;
 *         this.left = left;
 *         this.right = right;
 *     }
 * }
 */
class Solution {
    public TreeNode reverseOddLevels(TreeNode root) {
    //     if(root == null){
    //         return root;
    //     }
    //      reverseTree(root.left,root.right,1);
    //      return root;
    // }
    // public void reverseTree(TreeNode leftN, TreeNode rightN, int level){

    //     if(leftN==null || rightN==null){
    //         return;
    //     }
    //     if(level%2==1){
    //         int temp= leftN.val;
    //         leftN.val=rightN.val;
    //         rightN.val=temp;
    //     }

    //     reverseTree(leftN.left,rightN.right,level+1);
    //     reverseTree(leftN.right,leftN.left,level+1);


    // } if (root == null) return null;

        Queue<TreeNode> queue = new LinkedList<>();
        queue.offer(root);

        boolean isOddLevel = false; // Root is level 0, not odd

        while (!queue.isEmpty()) {
            int size = queue.size();
            List<TreeNode> levelNodes = new ArrayList<>();

            // Collect all nodes at the current level
            for (int i = 0; i < size; i++) {
                TreeNode node = queue.poll();
                levelNodes.add(node);

                if (node.left != null) queue.offer(node.left);
                if (node.right != null) queue.offer(node.right);
            }

            // Reverse values at odd levels
            if (isOddLevel) {
                int left = 0, right = levelNodes.size() - 1;
                while (left < right) {
                    int temp = levelNodes.get(left).val;
                    levelNodes.get(left).val = levelNodes.get(right).val;
                    levelNodes.get(right).val = temp;
                    left++;
                    right--;
                }
            }

            isOddLevel = !isOddLevel; // Toggle odd/even level
        }

        return root;
    }
}